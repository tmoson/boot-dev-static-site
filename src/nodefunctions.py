    ...
    for block in markdown_to_blocks(markdown):
        ...
        match block_type:
            case BlockType.CODE:
                children.append(
                    HTMLNode(
                        tag="pre",
                        children=[
                            text_node_to_html_node(TextNode(block[4:-4], TextType.CODE))
                        ],
                    )
                )
            case BlockType.QUOTE:
                children.append(
                    HTMLNode(
                        tag="blockquote", children=text_to_children(block[1:].strip())
                    )
                )
            case BlockType.HEADING:
                ...
                children.append(
                    HTMLNode(
                        tag=f"h{h_type}", children=text_to_children(block[h_type + 1 :])
                    )
                )
            case BlockType.ORDERED_LIST:
                ...
                for item in block.split("\n"):
                    list_items.append(
                        HTMLNode(
                            tag="li", children=text_to_children(item.split(" ", 1)[1])
                        )
                    )
            ...
            case BlockType.UNORDERED_LIST:
                ...
                for item in block.split("\n"):
                    list_items.append(
                        HTMLNode(tag="li", children=text_to_children(item[2:]))
                    )
